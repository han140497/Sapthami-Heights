...
export default async function CommitteeIssuesPage() {
  const [issues, costs, votes] = await Promise.all([getIssues(), getIssueCostSummary(), getIssueVotes()]);
    ...
    <>
      ...
      {issues.length === 0 ? (
        ...
        <div className="flex flex-col gap-8">
          <Group title={`Open (${open.length})`} issues={open} costByIssue={costByIssue} votes={votes} />
          {closed.length > 0 && <Group title={`Closed (${closed.length})`} issues={closed} costByIssue={costByIssue} votes={votes} />}
        </div>
      )}
    </>
  );
    ...
    <section>
      ...
      <div className="flex flex-col gap-2">
        {issues.map((issue) => {
          ...
          const voteCount = votes.get(issue.id)?.count ?? 0;
          return (
              ...
              <Card className="flex items-center justify-between gap-4 transition hover:border-accent">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {voteCount > 1 && (
                      <span className="flex shrink-0 items-center gap-1 rounded-full bg-accent/10 px-2 py-0.5 text-xs font-medium text-accent" title={`${voteCount} homes facing this`}>
                        <Users className="h-3 w-3" /> {voteCount}
                      </span>
                    )}
                  </div>
